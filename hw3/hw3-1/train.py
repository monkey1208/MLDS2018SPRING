...
def dcgan_train(data_loader, generator, discriminator, optimizer_G, optimizer_D, criterion, epoch):
    ...
    for e in range(epoch):
        ...
        for i, im in enumerate(tqdm(data_loader, desc= desc)):
            ...
            d_loss.backward()
            optimizer_D.step()
            im_fake_output = np.clip(im_fake.data.numpy(), 0, 1)
            if i % 40  == 0:
...
def wgan_train(data_loader, generator, discriminator, optimizer_G, optimizer_D, epoch, clip_val=0.01):
    Tensor = torch.FloatTensor
    for e in range(epoch):
        desc = 'Epoch {}'.format(e)
        for i, im in enumerate(tqdm(data_loader, desc= desc)):
...
def main(opts):
    ...
    if opts.use_wgan:
        print('WGAN Training')
        wgan_train(data_loader,generator,discriminator,optimizer_G,optimizer_D, opts.epoch)
    else:
